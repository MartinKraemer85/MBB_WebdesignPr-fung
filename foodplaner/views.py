...
from foodplaner.forms import foodItemForm
from foodplaner.models import Food, FoodEntry, FoodEntryItem


def get_calendar_days(year: Optional[int] = None, month: Optional[int] = None) -> list[list[date]]:
    ...
    calendar_start = first_day_of_month - timedelta(days=start_offset)
    days = [calendar_start + timedelta(days=i) for i in range(42)]

    return [days[i : i + 7] for i in range(0, 42, 7)]


def calendar(request: HttpRequest, year=None, month=None, day=None) -> HttpResponse:
    try:
    ...
    if request.META.get("HTTP_HX_REQUEST"):
        ...
        return render(
            request,
            "foodplaner/partials/calendar_htmx.html",
            context={
                "weeks": get_calendar_days(selected_date.year, selected_date.month),
...
def food_items(request: HttpRequest) -> HttpResponse:
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            print(form)
            return redirect("/foodplaner/food_items")

    form = foodItemForm()
    food_items = Food.objects.all()
    return render(
        request,
        "foodplaner/food_items.html",
        {
            "form": form,
            "food_items": food_items,
        },
...
def add_food_items(request: HttpRequest) -> HttpResponse:
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            print(form)
            return redirect("/foodplaner/add_food_items")

    form = foodItemForm()
    return render(request, "foodplaner/add_food_items.html", {"form": form})
...
def add_foodentry(request: HttpRequest) -> HttpResponse:
    ...
    return render(request, "foodplaner/partials/food_item_calendar.html", {"food_entry": food_entry})
